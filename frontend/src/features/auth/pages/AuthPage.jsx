...
function AuthPage() {
  ...
  return (
    <div className="bg-[#0E0500] text-white relative">
      <nav className="h-16 sm:h-20 flex shrink-0 justify-between items-center px-4 sm:px-11 border-b border-[#352101] backdrop-blur-xl sticky top-0 z-50">
        <Logo />
        <div className="bg-[#2C1E0D] border border-[#5a4626] p-1 rounded-4xl">
          <button
            className={activeView === "signin" ? activeBtn : inactiveBtn}
            onClick={() => dispatch(setView("signin"))}
          >
            Sign In
          </button>
          <button
            className={activeView === "signup" ? activeBtn : inactiveBtn}
