...
function AuthInfoPanel() {
  ...
  return (
    <div
      ...
      className="bg-cover bg-center hidden md:flex flex-col w-full md:w-[45%] min-h-screen md:sticky md:top-0 md:h-screen"
    >
      <div className="absolute bg-black/80 border-r border-[#462c03] z-10 inset-0"></div>
      <div className="relative z-20 px-8 pt-10 lg:px-16 lg:pt-16">
        <span className="border border-[#5a4626] text-[11px] lg:text-[13px] uppercase font-semibold tracking-widest px-4 lg:px-5 py-1.5 rounded-2xl bg-[#885e2b4b] text-[#F5A623] font-mono">
          Trusted by 10,000+ students
        </span>
        <h1
          style={{ fontFamily: "Playfair Display, serif" }}
          className="mt-8 lg:mt-12 mb-4 lg:mb-5 leading-tight text-[36px] lg:text-[54px] font-playfair font-black"
        >
          Your <span className="text-[#F5A623]">Perfect Room</span> Awaits You
        </h1>
        <p className="text-[#f0e3c77c] font-sans text-sm lg:text-[16px] w-full lg:w-[75%]">
          Verified PGs, hostels & flats near your campus. Zero brokerage, direct
          owner contact, honest prices.
        </p>
      </div>
      <div className="z-20 flex flex-col gap-4 px-8 pb-10 mt-8 lg:gap-5 lg:px-16 lg:pb-16 lg:mt-10">
        {badges.map((badge) => (
          <div
            key={badge.title}
            className="flex relative rounded-xl py-3 px-4 border border-[#5a4626] gap-3 items-center hover:border-[#F5A623] hover:translate-x-2 transition-all duration-400"
          >
