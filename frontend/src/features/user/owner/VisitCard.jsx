...
function VisitCard({ visit }) {
  // console.log(visit?.listing?.photos?.[0])
  return (
      ...
      <div className="flex justify-between pr-10">
        ...
        <div className="flex gap-6">
          <div className="w-20 h-20 bg-red-50 rounded-xl">
            <img src={visit?.listing?.photos?.[0]} className="object-cover overflow-hidden rounded-xl" />
          </div>
          <div className="flex flex-col gap-1">
            <p className="flex items-center gap-2">{visit.listing?.title}</p>
            <p className="flex items-center gap-2">
              <MapPin size={14} /> {visit.listing?.address?.locality}, {visit.listing?.address?.city}
            </p>
      ...
      <div className="flex flex-col justify-between border-l border-[#5a462691] pl-10">
        <div className="flex justify-between">
          <div className="flex flex-col gap-1 pb-2">
            ...
            <p className="text-sm italic">{visit.message && 
              <p>Note: "{visit.message}"</p>}</p>
          </div>
